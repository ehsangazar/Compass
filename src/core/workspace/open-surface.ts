...
export const WORKSPACE_GUIDANCE_START_MARKER = '<!-- COMPASS:WORKSPACE-GUIDANCE:START -->';
export const WORKSPACE_GUIDANCE_END_MARKER = '<!-- COMPASS:WORKSPACE-GUIDANCE:END -->';

export const WORKSPACE_GUIDANCE_BODY = `# Compass Workspace Guidance

This directory is an Compass workspace for planning across linked repos or folders.
...
- Use Compass workspace commands instead of hand-editing \`.compass-workspace/*.yaml\`.`;
...
export function applyWorkspaceGuidanceBlock(existingContent: string): string {
  ...
  if (startIndex !== -1 || endIndex !== -1) {
    if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
      throw new Error('Invalid Compass workspace guidance marker state in AGENTS.md.');
    }
