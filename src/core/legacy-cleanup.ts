/**
 * Legacy cleanup module for detecting and removing Compass artifacts
...
import { COMPASS_MARKERS } from './config.js';

/**
 * Legacy config file names from the old ToolRegistry.
 * These were config files created at project root with Compass markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not compass/AGENTS.md)
  'QWEN.md',
] as const;
...
export const LEGACY_SLASH_COMMAND_PATHS: Record<string, LegacySlashCommandPattern> = {
  // Directory-based: .tooldir/commands/compass/ or .tooldir/commands/compass/*.md
  'claude': { type: 'directory', path: '.claude/commands/compass' },
  'codebuddy': { type: 'directory', path: '.codebuddy/commands/compass' },
  'qoder': { type: 'directory', path: '.qoder/commands/compass' },
  'lingma': { type: 'directory', path: '.lingma/commands/compass' },
  'crush': { type: 'directory', path: '.crush/commands/compass' },
  'gemini': { type: 'directory', path: '.gemini/commands/compass' },
  'costrict': { type: 'directory', path: '.cospec/compass/commands' },

  // File-based: individual compass-*.md files in a commands/workflows/prompts folder
  'cursor': { type: 'files', pattern: '.cursor/commands/compass-*.md' },
  'windsurf': { type: 'files', pattern: '.windsurf/workflows/compass-*.md' },
  'kilocode': { type: 'files', pattern: '.kilocode/workflows/compass-*.md' },
  'kiro': { type: 'files', pattern: '.kiro/prompts/compass-*.prompt.md' },
  'github-copilot': { type: 'files', pattern: '.github/prompts/compass-*.prompt.md' },
  'amazon-q': { type: 'files', pattern: '.amazonq/prompts/compass-*.md' },
  'cline': { type: 'files', pattern: '.clinerules/workflows/compass-*.md' },
  'roocode': { type: 'files', pattern: '.roo/commands/compass-*.md' },
  'auggie': { type: 'files', pattern: '.augment/commands/compass-*.md' },
  'factory': { type: 'files', pattern: '.factory/commands/compass-*.md' },
  'opencode': { type: 'files', pattern: ['.opencode/command/compass-*.md', '.opencode/command/compass-*.md'] },
  'continue': { type: 'files', pattern: '.continue/prompts/compass-*.prompt' },
  'antigravity': { type: 'files', pattern: '.agent/workflows/compass-*.md' },
  'iflow': { type: 'files', pattern: '.iflow/commands/compass-*.md' },
  'junie': { type: 'files', pattern: ['.junie/commands/compass-*.md', '.junie/commands/compass-*.md'] },
  'qwen': { type: 'files', pattern: '.qwen/commands/compass-*.toml' },
  'codex': { type: 'files', pattern: '.codex/prompts/compass-*.md' },
};
...
export interface LegacyDetectionResult {
  /** Config files with Compass markers detected */
  configFiles: string[];
  ...
  /** Whether compass/AGENTS.md exists */
  hasCompassAgents: boolean;
  /** Whether compass/project.md exists (preserved, migration hint only) */
  hasProjectMd: boolean;
  /** Whether root AGENTS.md has Compass markers */
...
/**
 * Detects all legacy Compass artifacts in a project.
 *
...
export async function detectLegacyArtifacts(
  ...
  result.hasCompassAgents = structureResult.hasCompassAgents;
...
/**
 * Detects legacy config files with Compass markers.
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/compass-*.md'
...
async function findLegacySlashCommandFiles(
  ...
  try {
    ...
    // compass-*.prompt.md -> /^compass-.*\.prompt\.md$/
    // compass-*.toml -> /^compass-.*\.toml$/
...
/**
 * Detects legacy Compass structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  let hasCompassAgents = false;
  let hasProjectMd = false;
  let hasRootAgentsWithMarkers = false;

  // Check for compass/AGENTS.md
  const compassAgentsPath = FileSystemUtils.joinPath(projectPath, 'compass', 'AGENTS.md');
  hasCompassAgents = await FileSystemUtils.fileExists(compassAgentsPath);

  // Check for compass/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'compass', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with Compass markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasCompassMarkers(content);
  }

  return { hasCompassAgents, hasProjectMd, hasRootAgentsWithMarkers };
}

/**
 * Checks if content contains Compass markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasCompassMarkers(content: string): boolean {
  return (
    content.includes(COMPASS_MARKERS.start) && content.includes(COMPASS_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% Compass content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlyCompassContent(content: string): boolean {
  const startIndex = content.indexOf(COMPASS_MARKERS.start);
  const endIndex = content.indexOf(COMPASS_MARKERS.end);
  ...
  const after = content.substring(endIndex + COMPASS_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the Compass marker block from file content.
 ...
 *
 * @param content - File content with Compass markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, COMPASS_MARKERS.start, COMPASS_MARKERS.end);
}
...
/**
 * Cleans up legacy Compass artifacts from a project.
 * Preserves compass/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete legacy slash command directories (these are 100% Compass-managed)
  ...
  // Delete legacy slash command files (these are 100% Compass-managed)
  ...
  // Delete compass/AGENTS.md (this is inside compass/, it's Compass-managed)
  if (detection.hasCompassAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'compass', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('compass/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete compass/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with Compass markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const dir of result.deletedDirs) {
      lines.push(`  ✓ Removed ${dir}/ (replaced by /compass:*)`);
    }

    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed Compass markers from ${file}`);
    }
  }
 ...
 * Only includes Compass-managed files (slash commands, compass/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% Compass-managed)
  ...
  // Slash command files (these are 100% Compass-managed)
  for (const file of detection.slashCommandFiles) {
    removals.push({ path: file, explanation: 'replaced by skills/' });
  }

  // compass/AGENTS.md (inside compass/, it's Compass-managed)
  if (detection.hasCompassAgents) {
    removals.push({ path: 'compass/AGENTS.md', explanation: 'obsolete workflow file' });
  }
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing Compass markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new Compass'));
  lines.push('');
  lines.push('Compass now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('Compass markers will be removed, your content preserved:'));
  ...
  for (const file of detection.slashCommandFiles) {
    ...
    for (const [toolId, pattern] of Object.entries(LEGACY_SLASH_COMMAND_PATHS)) {
      if (pattern.type === 'files' && pattern.pattern) {
        ...
        // e.g., '.cursor/commands/compass-*.md' -> /^\.cursor\/commands\/compass-.*\.md$/
        const patterns = Array.isArray(pattern.pattern) ? pattern.pattern : [pattern.pattern];
        let matched = false;
        for (const p of patterns) {
...
export function formatProjectMdMigrationHint(): string {
  ...
  lines.push('  • compass/project.md');
  lines.push(chalk.dim('    We won\'t delete this file. It may contain useful project context.'));
  lines.push('');
  lines.push(chalk.dim('    The new compass/config.yaml has a "context:" section for planning'));
  lines.push(chalk.dim('    context. This is included in every Compass request and works more'));
