 ...
 * File path: .cursor/commands/compass-<id>.md
 * Frontmatter: name (as /compass-<id>), id, category, description
 */
export const cursorAdapter: ToolCommandAdapter = {
  toolId: 'cursor',

  getFilePath(commandId: string): string {
    return path.join('.cursor', 'commands', `compass-${commandId}.md`);
  },

  formatFile(content: CommandContent): string {
    return `---
name: /compass-${content.id}
id: compass-${content.id}
