 ...
 * File path: .pi/prompts/compass-<id>.md
 * Frontmatter: description
 *
 * Pi uses the filename (minus .md) as the slash command name, so
 * compass-propose.md → /compass-propose. Command references in the body
 * are transformed from /compass: to /compass- for consistency.
 */
export const piAdapter: ToolCommandAdapter = {
  toolId: 'pi',

  getFilePath(commandId: string): string {
    return path.join('.pi', 'prompts', `compass-${commandId}.md`);
  },

  formatFile(content: CommandContent): string {
    // Transform /compass: references to /compass- and inject $@ for template args
