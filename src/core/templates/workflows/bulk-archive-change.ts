...
export function getBulkArchiveChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'compass-bulk-archive-change',
...
1. **Get active changes**
   ...
   Run \`compass list --json\` to get all active changes.
...
3. **Batch validation - gather status for all selected changes**
   ...
   a. **Artifact status** - Run \`compass status --change "<name>" --json\`
...
8. **Execute archive for each confirmed change**
   ...
   a. **Sync specs** if delta specs exist:
      - Use the compass-sync-specs approach (agent-driven intelligent merge)
...
- Preserve .compass.yaml when moving to archive
