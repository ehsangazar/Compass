...
export class PowerShellInstaller {
  ...
  getInstallationPath(): string {
    ...
    return path.join(profileDir, 'CompassCompletion.ps1');
  }
  ...
  async configureProfile(scriptPath: string): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Add Compass completion configuration with markers
        const compassBlock = [
          '',
        ...
        const newContent = profileContent + compassBlock;
  ...
  async removeProfileConfig(): Promise<boolean> {
    ...
    for (const profilePath of profilePaths) {
      try {
        ...
        // Remove COMPASS:START -> COMPASS:END block
        const startMarker = '# COMPASS:START';
        const endMarker = '# COMPASS:END';
        const startIndex = profileContent.indexOf(startMarker);

        if (startIndex === -1) {
          continue; // No Compass block found
        }
