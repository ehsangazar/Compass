...
export class ZshInstaller {
  ...
  async getInstallationPath(): Promise<{ path: string; isOhMyZsh: boolean }> {
    ...
    if (isOhMyZsh) {
      ...
      return {
        path: path.join(this.homeDir, '.oh-my-zsh', 'custom', 'completions', '_compass'),
        isOhMyZsh: true,
      };
    } else {
      // Standard Zsh completions directory
      return {
        path: path.join(this.homeDir, '.zsh', 'completions', '_compass'),
        isOhMyZsh: false,
      };
    }
  }
  ...
  async configureZshrc(completionsDir: string): Promise<boolean> {
    ...
    if (process.env.COMPASS_NO_AUTO_CONFIG === '1') {
      return false;
    }
  ...
  /**
   * Check if .zshrc has Compass configuration markers
   *
  ...
  async uninstall(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (zshrcCleaned && !isOhMyZsh) {
        messages.push('Removed Compass configuration from ~/.zshrc');
      }
