...
/**
 * Generates Bash completion scripts for the Compass CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for Compass CLI
...
${helpers}
complete -F _compass_completion compass
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_compass_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_compass_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_compass_complete_items`);
        break;
      case 'schema-name':
        lines.push(`${indent}_compass_complete_schemas`);
        break;
      case 'shell':
