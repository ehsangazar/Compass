...
/**
 * Generates Zsh completion scripts for the Compass CLI.
 * Follows Zsh completion system conventions using the _compass function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(`          _compass_${this.sanitizeFunctionName(cmd.name)}`);
    ...
    return `#compdef compass

# Zsh completion script for Compass CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "compass command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
`;
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_compass_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _compass_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generateSubcommandFunction(parentName: string, subcmd: CommandDefinition): string[] {
    const funcName = `_compass_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_compass_complete_changes'";
      case 'spec-id':
        return "'*: :_compass_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_compass_complete_items'";
      case 'schema-name':
        return "'*: :_compass_complete_schemas'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
  ...
  private generateIndexedPositionalSpec(
    ...
    switch (positional.type) {
      case 'change-id':
        return `'${index}${separator}${name}:_compass_complete_changes'`;
      case 'spec-id':
        return `'${index}${separator}${name}:_compass_complete_specs'`;
      case 'change-or-spec-id':
        return `'${index}${separator}${name}:_compass_complete_items'`;
      case 'schema-name':
        return `'${index}${separator}${name}:_compass_complete_schemas'`;
      case 'path':
