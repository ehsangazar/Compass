...
/**
 * Generates Fish completion scripts for the Compass CLI.
...
export class FishGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      topLevelLines.push(
        `complete -c compass -n '__fish_compass_no_subcommand' -a '${cmd.name}' -d '${this.escapeDescription(cmd.description)}'`
      );
    }
    ...
    return `# Fish completion script for Compass CLI
...
${commandCompletions}`;
  ...
  private generateCommandCompletions(cmd: CommandDefinition): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        lines.push(
          `complete -c compass -n '__fish_compass_using_subcommand ${cmd.name}; and not __fish_compass_using_subcommand ${subcmd.name}' -a '${subcmd.name}' -d '${this.escapeDescription(subcmd.description)}'`
        );
      }
      lines.push('');
      ...
      for (const flag of cmd.flags) {
        lines.push(...this.generateFlagCompletion(flag, `__fish_compass_using_subcommand ${cmd.name}`));
      }
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        for (const flag of subcmd.flags) {
          lines.push(...this.generateFlagCompletion(flag, `__fish_compass_using_subcommand ${cmd.name}; and __fish_compass_using_subcommand ${subcmd.name}`));
        }

        // Add positional completions for subcommand
        if (subcmd.acceptsPositional) {
          lines.push(...this.generatePositionalCompletion(subcmd.positionalType, `__fish_compass_using_subcommand ${cmd.name}; and __fish_compass_using_subcommand ${subcmd.name}`));
        }
      }
    } else {
      ...
      if (cmd.acceptsPositional) {
        lines.push(...this.generatePositionalCompletion(cmd.positionalType, `__fish_compass_using_subcommand ${cmd.name}`));
      }
    }
  ...
  private generateFlagCompletion(flag: FlagDefinition, condition: string): string[] {
    ...
    if (flag.takesValue && flag.values) {
      ...
      for (const value of flag.values) {
        if (shortFlag) {
          lines.push(
            `complete -c compass -n '${condition}' -s ${flag.short} -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        } else {
          lines.push(
            `complete -c compass -n '${condition}' -l ${flag.name} -a '${value}' -d '${this.escapeDescription(flag.description)}'`
          );
        }
      }
      ...
      if (shortFlag) {
        lines.push(
          `complete -c compass -n '${condition}' -s ${flag.short} -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c compass -n '${condition}' -l ${flag.name} -r -d '${this.escapeDescription(flag.description)}'`
        );
      }
    } else {
      // Boolean flag
      if (shortFlag) {
        lines.push(
          `complete -c compass -n '${condition}' -s ${flag.short} -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      } else {
        lines.push(
          `complete -c compass -n '${condition}' -l ${flag.name} -d '${this.escapeDescription(flag.description)}'`
        );
      }
    }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, condition: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`complete -c compass -n '${condition}' -a '(__fish_compass_changes)' -f`);
        break;
      case 'spec-id':
        lines.push(`complete -c compass -n '${condition}' -a '(__fish_compass_specs)' -f`);
        break;
      case 'change-or-spec-id':
        lines.push(`complete -c compass -n '${condition}' -a '(__fish_compass_items)' -f`);
        break;
      case 'schema-name':
        lines.push(`complete -c compass -n '${condition}' -a '(__fish_compass_schemas)' -f`);
        break;
      case 'shell':
        lines.push(`complete -c compass -n '${condition}' -a 'zsh bash fish powershell' -f`);
        break;
      case 'path':
