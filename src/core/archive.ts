...
export class ArchiveCommand {
  async execute(
    ...
    const targetPath = '.';
    const changesDir = path.join(targetPath, 'compass', 'changes');
    const archiveDir = path.join(changesDir, 'archive');
    const mainSpecsDir = path.join(targetPath, 'compass', 'specs');

    // Check if changes directory exists
    try {
      await fs.access(changesDir);
    } catch {
      throw new Error("No Compass changes directory found. Run 'compass init' first.");
    }
