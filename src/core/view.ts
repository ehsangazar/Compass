...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const compassDir = path.join(targetPath, 'compass');
    
    if (!fs.existsSync(compassDir)) {
      console.error(chalk.red('No compass directory found'));
      process.exit(1);
    }
    ...
    const changesData = await this.getChangesData(compassDir);
    const specsData = await this.getSpecsData(compassDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('compass list --changes')} or ${chalk.white('compass list --specs')} for detailed views`));
  }

  private async getChangesData(compassDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(compassDir, 'changes');
  ...
  private async getSpecsData(compassDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(compassDir, 'specs');
