 ...
 * - Helps understand Compass's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other Compass schemas
 */
...
/**
 * Read and parse compass/config.yaml from project root.
 ...
 *
 * @param projectRoot - The root directory of the project (where `compass/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  let configPath = path.join(projectRoot, 'compass', 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, 'compass', 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`compass/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse compass/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in compass/config.yaml\n\n`;
  ...
  message += `\nFix: Edit compass/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
