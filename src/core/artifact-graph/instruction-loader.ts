 ...
 * 2. Schema from .compass.yaml metadata (if exists in change directory)
...
export function loadChangeContext(
  ...
  const changeDir = FileSystemUtils.canonicalizeExistingPath(
    options.changeDir ?? path.join(projectRoot, 'compass', 'changes', changeName)
  );
...
function buildNextSteps(
  ...
  if (readyArtifact) {
    steps.push(
      `Run compass instructions ${readyArtifact.id} --change "${context.changeName}" --json before writing that artifact.`
    );
