 ...
 * Init Command
 *
 * Sets up Compass with Agent Skills and /compass:* slash commands.
...
const WORKFLOW_TO_SKILL_DIR: Record<string, string> = {
  ...
  'new': 'compass-new-change',
  'continue': 'compass-continue-change',
  'apply': 'compass-apply-change',
  'ff': 'compass-ff-change',
  'sync': 'compass-sync-specs',
  'archive': 'compass-archive-change',
  'bulk-archive': 'compass-bulk-archive-change',
  'verify': 'compass-verify-change',
  'onboard': 'compass-onboard',
  'propose': 'compass-propose',
};
...
export class InitCommand {
  ...
  async execute(targetPath: string): Promise<void> {
    ...
    const compassDir = COMPASS_DIR_NAME;
    const compassPath = path.join(projectPath, compassDir);
    ...
    await this.createDirectoryStructure(compassPath, extendMode);

    // Generate skills and commands for each tool
    const results = await this.generateSkillsAndCommands(projectPath, validatedTools);

    // Create config.yaml if needed
    const configStatus = await this.createConfig(compassPath, extendMode);
  ...
  private async validate(
    ...
    const extendMode = await FileSystemUtils.directoryExists(compassPath);
  ...
  private async getSelectedTools(
    ...
    if (configuredNames.length > 0) {
      console.log(`Compass configured: ${configuredNames.join(', ')} (pre-selected)`);
    }
  ...
  private async createDirectoryStructure(compassPath: string, extendMode: boolean): Promise<void> {
    if (extendMode) {
      // In extend mode, just ensure directories exist without spinner
      const directories = [
        compassPath,
        path.join(compassPath, 'specs'),
        path.join(compassPath, 'changes'),
        path.join(compassPath, 'changes', 'archive'),
      ];
    ...
    const spinner = this.startSpinner('Creating Compass structure...');
    ...
    spinner.stopAndPersist({
      ...
      text: PALETTE.white('Compass structure created'),
    });
  }
  ...
  private async createConfig(compassPath: string, extendMode: boolean): Promise<'created' | 'exists' | 'skipped'> {
    const configPath = path.join(compassPath, 'config.yaml');
    const configYmlPath = path.join(compassPath, 'config.yml');
  ...
  private displaySuccessMessage(
    ...
    console.log();
    console.log(chalk.bold('Compass Setup Complete'));
    console.log();
    ...
    if (configStatus === 'created') {
      console.log(`Config: compass/config.yaml (schema: ${DEFAULT_SCHEMA})`);
    } else if (configStatus === 'exists') {
      // Show actual filename (config.yaml or config.yml)
      const configYaml = path.join(projectPath, COMPASS_DIR_NAME, 'config.yaml');
      const configYml = path.join(projectPath, COMPASS_DIR_NAME, 'config.yml');
      const configName = fs.existsSync(configYaml) ? 'config.yaml' : fs.existsSync(configYml) ? 'config.yml' : 'config.yaml';
      console.log(`Config: compass/${configName} (exists)`);
    } else {
    ...
    if (activeWorkflows.includes('propose')) {
      ...
      console.log('  Start your first change: /compass:propose "your idea"');
    } else if (activeWorkflows.includes('new')) {
      console.log(chalk.bold('Getting started:'));
      console.log('  Start your first change: /compass:new "your idea"');
    } else {
      console.log("Done. Run 'compass config profile' to configure your workflows.");
    }
