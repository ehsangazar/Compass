...
const METADATA_FILENAME = '.compass.yaml';
...
/**
 * Writes change metadata to .compass.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .compass.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .compass.yaml metadata (if exists)
 * 3. Schema from compass/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/compass/changes/change-name)
