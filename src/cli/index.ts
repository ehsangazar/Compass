...
program
  .command('init [path]')
  .description('Initialize Compass in your project')
...
program
  ...
  .action(async (options?: { tool?: string; noInteractive?: boolean }) => {
    try {
      console.log('Note: "compass experimental" is deprecated. Use "compass init" instead.');
      const { InitCommand } = await import('../core/init.js');
      const initCommand = new InitCommand({
        tools: options?.tool,
...
program
  .command('update [path]')
  .description('Update Compass instruction files')
...
program
  ...
  .option('--port <port>', 'Port to bind (default: 51234, auto-shifts if in use)')
...
dockerCommand
  ...
  .option('--port <port>', 'Host port to publish (default: 51234, auto-shifts if in use)')
...
const changeCmd = program
  .command('change')
  .description('Manage Compass change proposals');

// Deprecation notice for noun-based commands
changeCmd.hook('preAction', () => {
  console.error('Warning: The "compass change ..." commands are deprecated. Prefer verb-first commands (e.g., "compass list", "compass validate --changes").');
});
...
changeCmd
  .command('list')
  .description('List all active changes (DEPRECATED: use "compass list" instead)')
  .option('--json', 'Output as JSON')
  .option('--long', 'Show id and title with counts')
  .action(async (options?: { json?: boolean; long?: boolean }) => {
    try {
      console.error('Warning: "compass change list" is deprecated. Use "compass list".');
      const changeCommand = new ChangeCommand();
...
program
  ...
  .option('--concurrency <n>', 'Max concurrent validations (defaults to env COMPASS_CONCURRENCY or 6)')
  .option('--no-interactive', 'Disable interactive prompts')
...
program
  ...
  .description('Submit feedback about Compass')
...
const completionCmd = program
  .command('completion')
  .description('Manage shell completions for Compass CLI');
