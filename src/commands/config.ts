...
import { COMPASS_DIR_NAME } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for Compass',
  },
};
...
function maybeWarnProjectConfigDrift(
  ...
  const compassDir = path.join(projectDir, COMPASS_DIR_NAME);
  ...
  console.log(colorize('Warning: Global config is not applied to this project. Run `compass update` to sync.'));
}
...
async function maybeWarnConfigDrift(
  ...
  if (workspaceContext) {
    ...
    if (hasWorkspaceSkillProfileDrift(localState)) {
      console.log(
        colorize(
          'Warning: Workspace-local agent skills are out of sync with the active global profile. Run `compass workspace update` to sync.'
        )
      );
    }
    return;
...
function printConfigProfileApplyGuidance(workspaceContext: WorkspaceConfigProfileContext | null): void {
  if (workspaceContext) {
    console.log('Config updated. Run `compass workspace update` to apply it to workspace-local skills.');
    return;
  }

  console.log('Config updated. Run `compass update` in your projects to apply.');
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global Compass configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "compass config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: compass config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `compass config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `compass config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        if (workspaceContext) {
          ...
          if (applyNow) {
            try {
              execSync('npx compass workspace update', {
                stdio: 'inherit',
                cwd: workspaceContext.root,
              });
              console.log('Run `compass workspace update` in your other workspaces to apply.');
            } catch {
              console.error('`compass workspace update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
        ...
        // Check if inside an Compass project
        ...
        if (fs.existsSync(compassDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx compass update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `compass update` in your other projects to apply.');
            } catch {
              console.error('`compass update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
