...
function formatDuplicateLinkMessage(
  ...
  return [
    ...
    `  compass workspace link archived-${linkName} ${replacementPath}`,
    '',
    'If you meant to change the existing link path:',
    `  compass workspace relink ${linkName} ${replacementPath}`,
  ].join('\n');
}
...
function duplicateLinkError(
  ...
  return new WorkspaceCliError(
    ...
    'duplicate_link_name',
    {
      ...
      fix: `Choose a different link name or run 'compass workspace relink ${linkName} ${replacementPath}'.`,
    }
  );
}
...
function localStateInvalidStatus(error: unknown): WorkspaceStatus {
  return makeStatus(
    ...
    `Machine-local paths could not be read: ${asErrorMessage(error)}`,
    {
      ...
      fix: 'Repair or remove .compass-workspace/local.yaml, then run compass workspace relink <name> <path> for affected links.',
    }
  );
}
...
function workspaceSkillDriftStatus(workspaceName: string): WorkspaceStatus {
  return makeStatus(
    ...
    'Workspace-local agent skills are out of sync with the active global profile.',
    {
      ...
      fix: `compass workspace update --workspace ${workspaceName}`,
    }
  );
}
      ...
      workspace: {
        ...
        status: [
          makeStatus(
            ...
            {
              target: 'workspace.root',
              fix: 'Repair .compass-workspace/workspace.yaml before using this workspace.',
            }
          ),
        ],
      },
    ...
    if (!optionalLocalState) {
      workspaceStatus.push(
        makeStatus(
          ...
          {
            target: 'workspace.local_state',
            fix: 'Run compass workspace relink <name> <path> for each linked repo or folder on this machine.',
          }
        )
      );
    }
    ...
    workspaceStatus.push(
      makeStatus(
        ...
        'Workspace planning path does not exist.',
        {
          ...
          fix: `Create ${planningPath} or recreate the workspace with compass workspace setup.`,
        }
      )
    );
  }
  ...
  for (const linkName of linkNames) {
    ...
    if (!sharedNames.has(linkName)) {
      linkStatus.push(
        makeStatus(
          ...
          {
            target: `links.${linkName}`,
            fix: `Add a shared link with compass workspace link ${linkName} ${localPath ?? '/path/to/folder'} or remove the local-only path from .compass-workspace/local.yaml.`,
          }
        )
      );
    }
      ...
      linkStatus.push(
        makeStatus(
          ...
          'Shared link does not have a local path on this machine.',
          {
            ...
            fix: `compass workspace relink ${linkName} /path/to/${linkName}`,
          }
        )
      );
    }
    ...
    if (localPath) {
      if (await directoryExists(localPath)) {
        const candidateSpecsPath = path.join(localPath, 'compass', 'specs');
...
export async function readWorkspaceForMutation(
  ...
  if (!(await directoryExists(selected.root)) || !(await isWorkspaceRoot(selected.root))) {
    throw new WorkspaceCliError(
      ...
      {
        target: 'workspace.root',
        fix: 'Run compass workspace list to inspect known workspaces.',
      }
    );
  }
...
export async function updateWorkspaceLink(
  ...
  if (!sharedState.links[linkName]) {
    throw new WorkspaceCliError(`Unknown workspace link '${linkName}'.`, 'unknown_link_name', {
      target: `links.${linkName}`,
      fix: 'Run compass workspace doctor to see linked repos or folders.',
    });
  }
