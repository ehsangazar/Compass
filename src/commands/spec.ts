...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'compass' as const };
...
export class SpecCommand {
  private SPECS_DIR = 'compass/specs';
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at compass/specs/${specId}/spec.md`);
    }
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'compass' as const },
      };
...
export function registerSpecCommand(rootProgram: typeof program) {
  const specCommand = rootProgram
    .command('spec')
    .description('Manage and view Compass specifications');

  // Deprecation notice for noun-based commands
  specCommand.hook('preAction', () => {
    console.error('Warning: The "compass spec ..." commands are deprecated. Prefer verb-first commands (e.g., "compass show", "compass validate --specs").');
  });
