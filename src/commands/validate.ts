...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  compass validate --all');
    console.error('  compass validate --changes');
    console.error('  compass validate --specs');
    console.error('  compass validate <item-name>');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: compass change validate / compass spec validate');
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    if (type === 'change') {
      const changeDir = path.join(process.cwd(), 'compass', 'changes', id);
    ...
    const file = path.join(process.cwd(), 'compass', 'specs', id, 'spec.md');
    const start = Date.now();
  ...
  private printNextSteps(type: ItemType): void {
    ...
    if (type === 'change') {
      ...
      bullets.push('- Debug parsed deltas: compass change show <id> --json --deltas-only');
    } else {
  ...
  private async runBulkValidation(scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean }): Promise<void> {
    ...
    const concurrency = normalizeConcurrency(opts.concurrency) ?? normalizeConcurrency(process.env.COMPASS_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const validator = new Validator(opts.strict);
