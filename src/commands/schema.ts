  ...
  schemaCmd
    ...
    .action(async (
      ...
      try {
          ...
          if (!options?.force) {
            if (options?.json) {
              console.log(JSON.stringify({
                ...
                suggestion: 'Use --force to overwrite or "compass schema fork" to copy',
              }, null, 2));
            } else {
              console.error(`Error: Schema '${name}' already exists at ${schemaDir}`);
              console.error('Use --force to overwrite or "compass schema fork" to copy');
            }
            process.exitCode = 1;
            return;
          }
        ...
        if (options?.default) {
          const configPath = path.join(projectRoot, 'compass', 'config.yaml');
        ...
        if (options?.json) {
          ...
          console.log(`  3. Use with: compass new --schema ${name}`);
        }
      } catch (error) {
