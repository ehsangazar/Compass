...
async function promptWorkspaceSkillAgents(
  ...
  return searchableMultiSelect({
    message: 'Which agents should get Compass skills in this workspace?',
    pageSize: 15,
...
function printWorkspaceListHuman(workspaces: WorkspaceListOutput[]): void {
  console.log(chalk.bold(`Compass workspaces (${workspaces.length})`));
...
function assertWorkspaceOpenSupportedOptions(options: WorkspaceOpenOptions): void {
  if (options.prepareOnly) {
    throw new WorkspaceCliError(
      ...
      'workspace_open_prepare_only_unsupported',
      {
        ...
        fix: 'Run compass workspace open with --agent <tool> or --editor.',
      }
    );
  }
  ...
  if (options.json) {
    throw new WorkspaceCliError(
      ...
      {
        target: 'workspace.open',
        fix: 'Use compass workspace doctor --json for current workspace status.',
      }
    );
  }
...
function printWorkspaceOpenHuman(
  ...
  console.log('Repair skipped links with compass workspace doctor.');
}
...
class WorkspaceCommand {
    ...
    try {
      ...
      if (options.json && !noInteractive) {
        throw new WorkspaceCliError(
          ...
          {
            fix: 'compass workspace setup --no-interactive --json --name <name> --link <path>',
          }
        );
      }
      ...
      if (!interactive && (!options.name || (options.link ?? []).length === 0)) {
        throw new WorkspaceCliError(
          ...
          {
            fix: 'compass workspace setup --no-interactive --name platform --link /path/to/repo',
          }
        );
      }
      ...
      if (options.tools !== undefined) {
        ...
        console.log(chalk.dim('Choose which coding agents should get Compass skills in this workspace.'));
      ...
      const skillReport =
        selectedWorkspaceSkillAgents === undefined
          ? createWorkspaceSkillSkippedReport(
              'tools_omitted',
              'No workspace skills were installed. Run compass workspace update --tools <ids> to install them later.'
            )
      ...
      console.log(`  compass workspace doctor --workspace ${workspace.name}`);
      console.log(`  compass workspace update --workspace ${workspace.name} --tools <ids>`);
      console.log('  compass workspace list');
  ...
  async list(options: WorkspaceListOptions = {}): Promise<void> {
    try {
      ...
      if (workspaces.length === 0) {
        console.log("No Compass workspaces found. Run 'compass workspace setup' first.");
        return;
      }
    ...
    try {
      if (!nameOrPath) {
        throw new WorkspaceCliError(
          ...
          'missing_link_path',
          {
            fix: 'compass workspace link /path/to/repo',
          }
        );
      }
    ...
    try {
      if (!linkNameInput || !linkPath) {
        throw new WorkspaceCliError(
          ...
          {
            fix: 'compass workspace relink <name> /path/to/repo',
          }
        );
      }
  ...
  private async updateSelected(
    ...
    console.log('');
    console.log('Next useful commands:');
    console.log(`  compass workspace doctor --workspace ${doctorResult.workspace.name}`);
    console.log(`  compass workspace update --workspace ${doctorResult.workspace.name} --tools <ids>`);
...
export function registerWorkspaceCommand(program: Command): void {
  ...
  workspace
    ...
    .option(
      '--tools <tools>',
      `Install Compass skills for agents. Use "all", "none", or a comma-separated list of: ${getWorkspaceSkillToolIds().join(', ')}`
    )
    .option('--json', 'Output as JSON')
    .option('--no-interactive', 'Disable prompts')
  ...
  workspace
    .command('list')
    .description('List known Compass workspaces')
    .option('--json', 'Output as JSON')
  ...
  workspace
    .command('update [name]')
    .description('Refresh workspace-local Compass agent skills from the active global profile')
    .option('--workspace <name>', 'Workspace name from the local workspace registry')
    .option(
      '--tools <tools>',
