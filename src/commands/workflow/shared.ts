...
/**
 * Returns the list of available change directory names under compass/changes/.
 * Excludes the archive directory and hidden directories.
 */
export async function getAvailableChanges(
  projectRoot: string,
  changesDir = path.join(projectRoot, 'compass', 'changes')
): Promise<string[]> {
...
export async function validateChangeExists(
  ...
  if (!changeName) {
    ...
    if (available.length === 0) {
      throw new Error('No changes found. Create one with: compass new change <name>');
    }
    throw new Error(
  ...
  if (!exists) {
    ...
    if (available.length === 0) {
      throw new Error(
        `Change '${changeName}' not found. No changes exist. Create one with: compass new change <name>`
      );
    }
    throw new Error(
