 ...
 * `compass completion install` explicitly. This script only
 * prints a one-line tip after npm install.
 *
 * The tip is suppressed when:
 * - CI=true environment variable is set
 * - COMPASS_NO_COMPLETIONS=1 environment variable is set
...
function shouldSkipInstallation() {
  ...
  if (process.env.COMPASS_NO_COMPLETIONS === '1') {
    return { skip: true, reason: 'COMPASS_NO_COMPLETIONS=1 set' };
  }
...
async function main() {
  try {
    ...
    console.log(`\nTip: Run 'compass completion install' for shell completions`);
  } catch (error) {
