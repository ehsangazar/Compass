...
import { COMPASS_MARKERS } from '../../src/core/config.js';
...
describe('legacy-cleanup', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `compass-legacy-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });
    // Create compass directory structure
    await fs.mkdir(path.join(testDir, 'compass'), { recursive: true });
  });
  ...
  describe('hasCompassMarkers', () => {
    it('should return true when both markers are present', () => {
      const content = `Some content
${COMPASS_MARKERS.start}
Compass content
${COMPASS_MARKERS.end}
More content`;
      expect(hasCompassMarkers(content)).toBe(true);
    });
...
Compass content
${COMPASS_MARKERS.end}`;
      expect(hasCompassMarkers(content)).toBe(false);
    });

    it('should return false when end marker is missing', () => {
      const content = `${COMPASS_MARKERS.start}
Compass content
Some content`;
      expect(hasCompassMarkers(content)).toBe(false);
    });
  ...
  describe('isOnlyCompassContent', () => {
    it('should return true when content is only markers and whitespace outside', () => {
      const content = `${COMPASS_MARKERS.start}
Compass content here
${COMPASS_MARKERS.end}`;
      expect(isOnlyCompassContent(content)).toBe(true);
    });
...
Compass content
${COMPASS_MARKERS.end}`;
      expect(isOnlyCompassContent(content)).toBe(false);
    });
...
User content here`;
    ...
    it('should return false when end marker comes before start marker', () => {
      const content = `${COMPASS_MARKERS.end}
Content
${COMPASS_MARKERS.start}`;
      expect(isOnlyCompassContent(content)).toBe(false);
    });
...
${COMPASS_MARKERS.end}`;
      ...
      expect(result).not.toContain(COMPASS_MARKERS.start);
      expect(result).not.toContain(COMPASS_MARKERS.end);
    });
...
${COMPASS_MARKERS.start}`;
      const result = removeMarkerBlock(content);
      expect(result).toContain(COMPASS_MARKERS.end);
      expect(result).toContain(COMPASS_MARKERS.start);
    });

    it('should ignore inline mentions of markers and only remove actual block', () => {
      const content = `Intro referencing ${COMPASS_MARKERS.start} and ${COMPASS_MARKERS.end} inline.
...
After content`;
  ...
  describe('detectLegacyConfigFiles', () => {
    it('should detect CLAUDE.md with Compass markers and put in update list', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
      await fs.writeFile(claudePath, `${COMPASS_MARKERS.start}
Compass content
${COMPASS_MARKERS.end}`);
...
${COMPASS_MARKERS.end}`);
    ...
    it('should not detect files without Compass markers', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
    ...
    it('should detect multiple config files', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'CLAUDE.md'), `${COMPASS_MARKERS.start}\nContent\n${COMPASS_MARKERS.end}`);
      await fs.writeFile(path.join(testDir, 'CLINE.md'), `${COMPASS_MARKERS.start}\nContent\n${COMPASS_MARKERS.end}`);
      await fs.writeFile(path.join(testDir, 'QODER.md'), `${COMPASS_MARKERS.start}\nContent\n${COMPASS_MARKERS.end}`);
  ...
  describe('detectLegacySlashCommands', () => {
    it('should detect legacy Claude slash command directory', async () => {
      const dirPath = path.join(testDir, '.claude', 'commands', 'compass');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'proposal.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain('.claude/commands/compass');
    });

    it('should detect legacy Cursor slash command files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-proposal.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'compass-apply.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.cursor/commands/compass-proposal.md');
      expect(result.files).toContain('.cursor/commands/compass-apply.md');
    });

    it('should detect legacy Windsurf workflow files', async () => {
      const dirPath = path.join(testDir, '.windsurf', 'workflows');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-archive.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.windsurf/workflows/compass-archive.md');
    });

    it('should detect multiple tool directories and files', async () => {
      // Create directory-based
      await fs.mkdir(path.join(testDir, '.claude', 'commands', 'compass'), { recursive: true });
      await fs.mkdir(path.join(testDir, '.qoder', 'commands', 'compass'), { recursive: true });

      // Create file-based
      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(path.join(testDir, '.cursor', 'commands', 'compass-proposal.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.directories).toContain('.claude/commands/compass');
      expect(result.directories).toContain('.qoder/commands/compass');
      expect(result.files).toContain('.cursor/commands/compass-proposal.md');
    });

    it('should not detect non-compass files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
    ...
    it('should detect TOML-based slash commands for Qwen', async () => {
      ...
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-proposal.toml'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.qwen/commands/compass-proposal.toml');
    });

    it('should detect Continue prompt files', async () => {
      const dirPath = path.join(testDir, '.continue', 'prompts');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-apply.prompt'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.continue/prompts/compass-apply.prompt');
    });

    it('should detect legacy OpenCode compass-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-propose.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/compass-propose.md');
    });

    it('should detect legacy OpenCode compass-* command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-new.md'), 'content');

      const result = await detectLegacySlashCommands(testDir);
      expect(result.files).toContain('.opencode/command/compass-new.md');
    });

    it('should detect both compass-* and compass-* OpenCode command files', async () => {
      const dirPath = path.join(testDir, '.opencode', 'command');
      await fs.mkdir(dirPath, { recursive: true });
      await fs.writeFile(path.join(dirPath, 'compass-propose.md'), 'content');
      await fs.writeFile(path.join(dirPath, 'compass-new.md'), 'content');
  ...
  describe('detectLegacyStructureFiles', () => {
    it('should detect compass/AGENTS.md', async () => {
      const agentsPath = path.join(testDir, 'compass', 'AGENTS.md');
      await fs.writeFile(agentsPath, '# AGENTS.md content');

      const result = await detectLegacyStructureFiles(testDir);
      expect(result.hasCompassAgents).toBe(true);
    });

    it('should detect compass/project.md', async () => {
      const projectPath = path.join(testDir, 'compass', 'project.md');
    ...
    it('should detect root AGENTS.md with Compass markers', async () => {
      const agentsPath = path.join(testDir, 'AGENTS.md');
      await fs.writeFile(agentsPath, `${COMPASS_MARKERS.start}
Compass content
${COMPASS_MARKERS.end}`);
    ...
    it('should handle non-existent files gracefully', async () => {
      const result = await detectLegacyStructureFiles(testDir);
      expect(result.hasCompassAgents).toBe(false);
  ...
  describe('detectLegacyArtifacts', () => {
    ...
    it('should return hasLegacyArtifacts: true when slash commands are found', async () => {
      ...
      const result = await detectLegacyArtifacts(testDir);
      expect(result.hasLegacyArtifacts).toBe(true);
      expect(result.slashCommandDirs).toContain('.claude/commands/compass');
    });

    it('should return hasLegacyArtifacts: true when compass/AGENTS.md is found', async () => {
      await fs.writeFile(path.join(testDir, 'compass', 'AGENTS.md'), 'content');
    ...
    it('should detect project.md for migration hint (it is preserved, not deleted)', async () => {
      await fs.writeFile(path.join(testDir, 'compass', 'project.md'), 'content');
  ...
  describe('cleanupLegacyArtifacts', () => {
    it('should remove markers from config files that have only Compass content (never delete)', async () => {
      const claudePath = path.join(testDir, 'CLAUDE.md');
      await fs.writeFile(claudePath, `${COMPASS_MARKERS.start}\nContent\n${COMPASS_MARKERS.end}`);
      ...
      expect(content).not.toContain(COMPASS_MARKERS.start);
      expect(content).not.toContain(COMPASS_MARKERS.end);
    });
...
${COMPASS_MARKERS.end}`);
    ...
    it('should delete legacy slash command directories', async () => {
      ...
      expect(result.deletedDirs).toContain('.claude/commands/compass');
    ...
    it('should delete legacy slash command files', async () => {
      const dirPath = path.join(testDir, '.cursor', 'commands');
      await fs.mkdir(dirPath, { recursive: true });
      const filePath = path.join(dirPath, 'compass-proposal.md');
      await fs.writeFile(filePath, 'content');

      const detection = await detectLegacyArtifacts(testDir);
      const result = await cleanupLegacyArtifacts(testDir, detection);

      expect(result.deletedFiles).toContain('.cursor/commands/compass-proposal.md');
      await expect(fs.access(filePath)).rejects.toThrow();
    });

    it('should delete compass/AGENTS.md', async () => {
      const agentsPath = path.join(testDir, 'compass', 'AGENTS.md');
      ...
      expect(result.deletedFiles).toContain('compass/AGENTS.md');
      await expect(fs.access(agentsPath)).rejects.toThrow();
      // compass directory should still exist
      await expect(fs.access(path.join(testDir, 'compass'))).resolves.not.toThrow();
    });

    it('should NOT delete compass/project.md', async () => {
      const projectPath = path.join(testDir, 'compass', 'project.md');
      ...
      expect(result.deletedFiles).not.toContain('compass/project.md');
...
${COMPASS_MARKERS.end}`);
    ...
    it('should remove markers from root AGENTS.md even when only Compass content (never delete)', async () => {
      const agentsPath = path.join(testDir, 'AGENTS.md');
      await fs.writeFile(agentsPath, `${COMPASS_MARKERS.start}\nCompass content\n${COMPASS_MARKERS.end}`);
  ...
  describe('formatCleanupSummary', () => {
    ...
    it('should format deleted directories', () => {
      const result = {
        deletedFiles: [],
        modifiedFiles: [],
        deletedDirs: ['.claude/commands/compass'],
        projectMdNeedsMigration: false,
        errors: [],
      };

      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removed .claude/commands/compass/ (replaced by /compass:*)');
    });
    ...
    it('should format modified files', () => {
      ...
      const summary = formatCleanupSummary(result);
      expect(summary).toContain('✓ Removed Compass markers from AGENTS.md');
    });
    ...
    it('should include migration hint for project.md', () => {
      ...
      expect(summary).toContain('compass/project.md');
  ...
  describe('formatDetectionSummary', () => {
    it('should include welcoming upgrade header and explanation', () => {
      ...
      expect(summary).toContain('Upgrading to the new Compass');
    ...
    it('should format slash command directories', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: ['.claude/commands/compass'],
        slashCommandFiles: [],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
      ...
      expect(summary).toContain('• .claude/commands/compass/');
    });
    ...
    it('should format slash command files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.cursor/commands/compass-proposal.md'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('• .cursor/commands/compass-proposal.md');
    });

    it('should format compass/AGENTS.md', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [],
        hasCompassAgents: true,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
      };
      ...
      const summary = formatDetectionSummary(detection);
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('• compass/AGENTS.md');
    });
    ...
    it('should group both removals and updates correctly', () => {
      ...
      expect(summary).toContain('Files to remove');
      expect(summary).toContain('Files to update');
      // Check removals (only slash commands and compass/AGENTS.md)
      expect(summary).toContain('• .claude/commands/compass/');
      expect(summary).toContain('• compass/AGENTS.md');
  ...
  describe('formatProjectMdMigrationHint', () => {
    it('should return migration hint message', () => {
      ...
      expect(hint).toContain('compass/project.md');
    ...
    it('should explain the new context section benefits', () => {
      const hint = formatProjectMdMigrationHint();
      expect(hint).toContain('included in every Compass request');
  ...
  describe('LEGACY_SLASH_COMMAND_PATHS', () => {
    it('should include expected tool patterns', () => {
      expect(LEGACY_SLASH_COMMAND_PATHS['claude']).toEqual({
        type: 'directory',
        path: '.claude/commands/compass',
      });

      expect(LEGACY_SLASH_COMMAND_PATHS['cursor']).toEqual({
        type: 'files',
        pattern: '.cursor/commands/compass-*.md',
      });

      expect(LEGACY_SLASH_COMMAND_PATHS['windsurf']).toEqual({
        type: 'files',
        pattern: '.windsurf/workflows/compass-*.md',
      });
    });
  ...
  describe('getToolsFromLegacyArtifacts', () => {
    ...
    it('should extract multiple tools from mixed legacy artifacts', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: ['.claude/commands/compass', '.qoder/commands/compass'],
        slashCommandFiles: ['.cursor/commands/compass-apply.md', '.windsurf/workflows/compass-archive.md'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should deduplicate tools when multiple files match same tool', () => {
      const detection = {
        ...
        slashCommandDirs: [],
        slashCommandFiles: [
          '.cursor/commands/compass-proposal.md',
          '.cursor/commands/compass-apply.md',
          '.cursor/commands/compass-archive.md',
        ],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
    ...
    it('should handle qwen TOML-based legacy files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.qwen/commands/compass-proposal.toml'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle continue prompt files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.continue/prompts/compass-apply.prompt'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle github-copilot prompt files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.github/prompts/compass-apply.prompt.md'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle opencode compass-* legacy files', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/compass-propose.md'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should handle opencode compass-* legacy files', () => {
      const detection = {
        ...
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: ['.opencode/command/compass-new.md'],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
        hasLegacyArtifacts: true,
    ...
    it('should deduplicate opencode when both compass-* and compass-* files exist', () => {
      const detection = {
        configFiles: [],
        configFilesToUpdate: [],
        slashCommandDirs: [],
        slashCommandFiles: [
          '.opencode/command/compass-propose.md',
          '.opencode/command/compass-new.md',
        ],
        hasCompassAgents: false,
        hasProjectMd: false,
        hasRootAgentsWithMarkers: false,
