...
describe('workspace foundation', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'compass-workspace-foundation-'));
...
`
  ...
  describe('path helpers', () => {
    it('exposes the workspace constants', () => {
      expect(WORKSPACE_METADATA_DIR_NAME).toBe('.compass-workspace');
    ...
    it('returns workspace file paths using platform-aware path helpers', () => {
      ...
      expect(getWorkspaceMetadataDir(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.compass-workspace')
      );
      expect(getWorkspaceSharedStatePath(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.compass-workspace', 'workspace.yaml')
      );
      expect(getWorkspaceLocalStatePath(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.compass-workspace', 'local.yaml')
      );
    ...
    it('preserves Windows-style location strings when building workspace file paths', () => {
      ...
      expect(getWorkspaceSharedStatePath(workspaceRoot)).toBe(
        'D:\\repos\\platform-workspace\\.compass-workspace\\workspace.yaml'
      );
      expect(getWorkspaceLocalStatePath(workspaceRoot)).toBe(
        'D:\\repos\\platform-workspace\\.compass-workspace\\local.yaml'
      );
    });
    ...
    it('uses getGlobalDataDir for managed workspace and registry locations', () => {
      ...
      expect(getManagedWorkspacesDir()).toBe(path.join(tempDir, 'compass', 'workspaces'));
      expect(getManagedWorkspaceRoot('platform')).toBe(
        path.join(tempDir, 'compass', 'workspaces', 'platform')
      );
      expect(getWorkspaceRegistryPath()).toBe(
        path.join(tempDir, 'compass', 'workspaces', 'registry.yaml')
      );
    });
    ...
    it('uses the Linux data-dir fallback under the managed workspaces directory', () => {
      ...
      expect(getManagedWorkspacesDir({ globalDataDir: dataDir })).toBe(
        '/home/tabish/.local/share/compass/workspaces'
      );
    });
    ...
    it('uses the native Windows data-dir fallback under the managed workspaces directory', () => {
      ...
      expect(getManagedWorkspacesDir({ globalDataDir: dataDir })).toBe(
        'C:\\Users\\Tabish\\AppData\\Local\\compass\\workspaces'
      );
    });

    it('exposes the portable collaboration ignore rule for local state', () => {
      expect(WORKSPACE_LOCAL_STATE_IGNORE_PATTERN).toBe('.compass-workspace/local.yaml');
      expect(getWorkspacePortableIgnorePatterns()).toEqual(['.compass-workspace/local.yaml']);
      expect(getWorkspacePortableIgnorePatterns('platform')).toEqual([
        '.compass-workspace/local.yaml',
  ...
  describe('workspace folder detection', () => {
    ...
    it('does not mistake repo-local compass projects for coordination workspaces', async () => {
      const repoRoot = path.join(tempDir, 'repo');
      fs.mkdirSync(path.join(repoRoot, 'compass', 'changes', 'add-feature'), {
        recursive: true,
      });
      fs.mkdirSync(path.join(repoRoot, 'compass', 'specs'), { recursive: true });

      await expect(findWorkspaceRoot(path.join(repoRoot, 'compass', 'changes'))).resolves.toBe(
        null
      );
    });

    it('detects a workspace even when a linked path has no repo-local compass state', async () => {
      const workspaceRoot = createWorkspaceRoot();
...
`;
      ...
      expect(refreshed.match(/COMPASS:WORKSPACE-GUIDANCE:START/gu)).toHaveLength(1);
      expect(applyWorkspaceGuidanceBlock('# Team Notes\n')).toContain(
        '<!-- COMPASS:WORKSPACE-GUIDANCE:START -->'
      );
    });
...
`);
    ...
    it('reads the local registry from the standard registry path', async () => {
      const globalDataDir = path.join(tempDir, 'data', 'compass');
