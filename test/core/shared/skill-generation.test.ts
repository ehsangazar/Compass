...
describe('skill-generation', () => {
  describe('getSkillTemplates', () => {
    ...
    it('should include all expected skills', () => {
      ...
      expect(dirNames).toContain('compass-explore');
      expect(dirNames).toContain('compass-new-change');
      expect(dirNames).toContain('compass-continue-change');
      expect(dirNames).toContain('compass-apply-change');
      expect(dirNames).toContain('compass-ff-change');
      expect(dirNames).toContain('compass-sync-specs');
      expect(dirNames).toContain('compass-archive-change');
      expect(dirNames).toContain('compass-bulk-archive-change');
      expect(dirNames).toContain('compass-verify-change');
      expect(dirNames).toContain('compass-onboard');
      expect(dirNames).toContain('compass-propose');
    });
    ...
    it('should return single template when filter has one workflow', () => {
      ...
      expect(filtered[0].dirName).toBe('compass-propose');
    });
  });
  ...
  describe('generateSkillContent', () => {
    ...
    it('should use default values for optional fields', () => {
      ...
      expect(content).toContain('license: MIT');
      expect(content).toContain('compatibility: Requires compass CLI.');
      expect(content).toContain('author: compass');
    ...
    it('should apply transformInstructions callback when provided', () => {
      const template = {
        ...
        instructions: 'Use /compass:new to start and /compass:apply to implement.',
      };

      const transformer = (text: string) => text.replace(/\/compass:/g, '/compass-');
      const content = generateSkillContent(template, '0.23.0', transformer);

      expect(content).toContain('/compass-new');
      expect(content).toContain('/compass-apply');
      expect(content).not.toContain('/compass:new');
      expect(content).not.toContain('/compass:apply');
    });
    ...
    it('should not transform instructions when callback is undefined', () => {
      const template = {
        ...
        instructions: 'Use /compass:new to start.',
      };
