...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `compass-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing compass/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No Compass changes directory found. Run 'compass init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'compass', 'changes');
