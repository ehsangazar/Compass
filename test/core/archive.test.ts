...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `compass-archive-test-${Date.now()}`);
    ...
    const compassDir = path.join(tempDir, 'compass');
    await fs.mkdir(path.join(compassDir, 'changes'), { recursive: true });
    await fs.mkdir(path.join(compassDir, 'specs'), { recursive: true });
    await fs.mkdir(path.join(compassDir, 'changes', 'archive'), { recursive: true });
  ...
  describe('execute', () => {
    it('should archive a change successfully', async () => {
      ...
      const changeDir = path.join(tempDir, 'compass', 'changes', changeName);
      ...
      const archiveDir = path.join(tempDir, 'compass', 'changes', 'archive');
...
Then expected result happens`;
      ...
      const mainSpecPath = path.join(tempDir, 'compass', 'specs', 'test-capability', 'spec.md');
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, 'compass', 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, 'compass', 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'compass', 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, 'compass', 'changes', 'archive', `${date}-${changeName}`);
...
Then expected result happens`;
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'compass', 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'compass', 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'compass', 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it('should abort with a structural error when target spec hides requirements outside ## Requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'compass', 'specs', 'delta-target');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** qux changes`;
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, 'compass', 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, 'compass', 'specs', 'epsilon', 'spec.md');
...
e1`);
      ...
      const zetaMain = path.join(tempDir, 'compass', 'specs', 'zeta', 'spec.md');
...
### Requirement: Missing`);
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, 'compass', 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, 'compass', 'specs', 'psi', 'spec.md');
  ...
  describe('error handling', () => {
    it('should throw error when compass directory does not exist', async () => {
      // Remove compass directory
      await fs.rm(path.join(tempDir, 'compass'), { recursive: true });
      
      await expect(
        archiveCommand.execute('any-change', { yes: true })
      ).rejects.toThrow("No Compass changes directory found. Run 'compass init' first.");
    });
  });
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'compass', 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'compass', 'changes', change2), { recursive: true });
