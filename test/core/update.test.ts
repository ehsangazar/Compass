...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `compass-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });

    // Create compass directory
    const compassDir = path.join(testDir, 'compass');
    await fs.mkdir(compassDir, { recursive: true });
  ...
  describe('basic validation', () => {
    it('should throw error if compass directory does not exist', async () => {
      // Remove compass directory
      await fs.rm(path.join(testDir, 'compass'), {
        recursive: true,
        force: true,
      });

      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No Compass directory found. Run 'compass init' first."
      );
    });
  ...
  describe('skill updates', () => {
    it('should update skill files for configured Claude tool', async () => {
      ...
      const exploreSkillDir = path.join(skillsDir, 'compass-explore');
      await fs.mkdir(exploreSkillDir, { recursive: true });

      // Create an existing skill file
      const oldSkillContent = `---
name: compass-explore (old)
description: Old description
license: MIT
compatibility: Requires compass CLI.
metadata:
  author: compass
  version: "0.9"
---
...
`;
      ...
      expect(updatedSkill).toContain('name: compass-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'compass-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'compass-explore', 'SKILL.md'),
        'old content'
      );
      ...
      const coreSkillNames = [
        'compass-explore',
        'compass-apply-change',
        'compass-sync-specs',
        'compass-archive-change',
        'compass-propose',
      ];
      ...
      const nonCoreSkillNames = [
        'compass-new-change',
        'compass-continue-change',
        'compass-ff-change',
        'compass-bulk-archive-change',
        'compass-verify-change',
      ];
  ...
  describe('command updates', () => {
    it('should update compass commands for configured Claude tool', async () => {
      ...
      // Check compass command files were created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'compass');
    ...
    it('should update core profile compass commands when tool is configured', async () => {
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'compass-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'compass-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'compass-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'compass-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: compass-explore');
      ...
      expect(cursorSkill).toContain('name: compass-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'compass-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'compass-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: compass-<id>.toml
      const qwenCmd = path.join(
        testDir,
        '.qwen',
        'commands',
        'compass-explore.toml'
      );
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'compass-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'compass-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'compass-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'compass-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'compass-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'compass-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'compass-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with Compass markers
      const legacyContent = `${COMPASS_MARKERS.start}
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new Compass')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed Compass markers from CLAUDE.md')
      );
...
`;
    ...
    it('should cleanup legacy slash command directories with --force', async () => {
      ...
      const legacyCommandDir = path.join(testDir, '.claude', 'commands', 'compass');
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed .claude/commands/compass/')
      );
    ...
    it('should cleanup legacy compass/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'compass-explore'), {
        recursive: true,
      ...
      // Create legacy compass/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'compass', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed compass/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new Compass')
      );
    ...
    it('should remove Compass marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'compass-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + Compass markers)
...
`;
      ...
      expect(updatedContent).not.toContain(COMPASS_MARKERS.start);
      expect(updatedContent).not.toContain(COMPASS_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/compass:new')
      );

      // Skills should be created
      const skillFile = path.join(testDir, '.claude', 'skills', 'compass-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.claude', 'commands', 'compass', 'proposal.md'),
        'content'
      );

      await fs.mkdir(path.join(testDir, '.cursor', 'commands'), { recursive: true });
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'compass-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'compass-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'compass-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'compass-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'compass-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'compass-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'compass-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'compass-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New compass commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'compass');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'compass-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'compass-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'compass-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'compass-new-change', 'SKILL.md')
      )).toBe(true);
    ...
    it('should suggest core preset when custom profile preserves the old core workflow set', async () => {
      ...
      expect(calls.some(call =>
        call.includes('compass config profile core') && call.includes('compass update')
      )).toBe(true);

      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'skills', 'compass-sync-specs', 'SKILL.md')
      )).toBe(false);
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'commands', 'compass', 'sync.md')
      )).toBe(false);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'compass-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'compass-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'compass', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'compass-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'compass-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'compass init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'compass init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'compass-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'compass-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'compass-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'compass-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'compass-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'compass-apply-change', 'SKILL.md'), 'content');
