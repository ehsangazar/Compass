...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `compass-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `compass-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'compass-explore');
    await writeSkill(projectDir, 'compass-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'compass', 'my-custom.md');
