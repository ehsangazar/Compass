...
describe('FishInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `compass-fish-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard fish completions path', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'compass.fish'));
    });
    ...
    it('should use homeDir from constructor', () => {
      ...
      expect(result).toBe(path.join(customHome, '.config', 'fish', 'completions', 'compass.fish'));
    });
  });
  ...
  describe('install', () => {
    const mockCompletionScript = `# Fish completion script for Compass CLI
function __fish_compass
    echo "test"
end

complete -c compass -a 'init' -d 'Initialize Compass'
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'compass.fish'));
    ...
    it('should create parent directories if they do not exist', async () => {
      ...
      expect(result.success).toBe(true);
      const targetPath = path.join(testHomeDir, '.config', 'fish', 'completions', 'compass.fish');
    ...
    it('should update when content is different', async () => {
      ...
      const updatedScript = `# Fish completion script for Compass CLI
function __fish_compass_new
    echo "updated"
end

complete -c compass -a 'init' -d 'Initialize Compass'
complete -c compass -a 'validate' -d 'Validate specs'
`;
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `compass fish test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('compass fish test');
