...
describe('ZshInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = path.join(os.tmpdir(), `compass-zsh-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return Oh My Zsh path when Oh My Zsh is installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.oh-my-zsh', 'custom', 'completions', '_compass'));
    });
    ...
    it('should return standard Zsh path when Oh My Zsh is not installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.zsh', 'completions', '_compass'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '#compdef compass\n_compass() {\n  echo "test"\n}\n';
    ...
    it('should install to Oh My Zsh path when Oh My Zsh is present', async () => {
      ...
      expect(result.isOhMyZsh).toBe(true);
      expect(result.installedPath).toBe(path.join(ohMyZshPath, 'custom', 'completions', '_compass'));
    ...
    it('should install to standard Zsh path when Oh My Zsh is not present', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.isOhMyZsh).toBe(false);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.zsh', 'completions', '_compass'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.zsh', 'completions', '_compass');
    ...
    it('should include fpath instructions for standard Zsh when auto-config is disabled', async () => {
      const originalEnv = process.env.COMPASS_NO_AUTO_CONFIG;
      process.env.COMPASS_NO_AUTO_CONFIG = '1';
      ...
      if (originalEnv === undefined) {
        delete process.env.COMPASS_NO_AUTO_CONFIG;
      } else {
        process.env.COMPASS_NO_AUTO_CONFIG = originalEnv;
      }
    });
    ...
    it('should handle paths with spaces in .zshrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `compass zsh test ${randomUUID()}`);
  ...
  describe('uninstall', () => {
    ...
    it('should remove from correct location for Oh My Zsh', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(path.join('.oh-my-zsh', 'custom', 'completions', '_compass'));
    });
  });
  ...
  describe('getInstallationInfo', () => {
    ...
    it('should return installation info when installed', async () => {
      ...
      expect(info.path).toContain('_compass');
  ...
  describe('configureZshrc', () => {
    ...
    it('should create .zshrc with markers and config when file does not exist', async () => {
      ...
      expect(content).toContain('# COMPASS:START');
      expect(content).toContain('# COMPASS:END');
      expect(content).toContain('# Compass shell completions configuration');
    ...
    it('should prepend markers and config when .zshrc exists without markers', async () => {
      ...
      const configIndex = content.indexOf('# COMPASS:START');
    ...
    it('should preserve user content outside markers', async () => {
      ...
      expect(content).toContain(`fpath=("${completionsDir}" $fpath)`);
      expect(content).not.toContain('# Old Compass config');
    });

    it('should return false when COMPASS_NO_AUTO_CONFIG is set', async () => {
      const originalEnv = process.env.COMPASS_NO_AUTO_CONFIG;
      process.env.COMPASS_NO_AUTO_CONFIG = '1';
  ...
  describe('removeZshrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '',
        '# COMPASS:START',
        '# Compass shell completions configuration',
      ...
      expect(newContent).not.toContain('# COMPASS:START');
      expect(newContent).not.toContain('# COMPASS:END');
      expect(newContent).not.toContain('Compass shell completions');
    ...
    it('should handle invalid marker placement gracefully', async () => {
      ...
      await fs.writeFile(zshrcPath, '# COMPASS:END\n# COMPASS:START\n');
    ...
    it('should return true when only one marker is present', async () => {
      const zshrcPath = path.join(testHomeDir, '.zshrc');
      await fs.writeFile(zshrcPath, '# COMPASS:START\nsome config\n');
  ...
  describe('install with .zshrc auto-configuration', () => {
    ...
    it('should not configure .zshrc for Oh My Zsh', async () => {
      ...
      expect(content).not.toContain('# COMPASS:START');
  ...
  describe('uninstall with .zshrc cleanup', () => {
    ...
    it('should remove .zshrc config when uninstalling', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain('Removed Compass configuration from ~/.zshrc');
