...
describe('BashInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = path.join(os.tmpdir(), `compass-bash-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard bash-completion path', async () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'compass'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '# Bash completion script for Compass CLI\n_compass_completion() {\n  echo "test"\n}\n';

    it('should install to bash-completion path', async () => {
      const result = await installer.install(testScript);

      expect(result.success).toBe(true);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'compass'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'compass');
    ...
    it('should handle paths with spaces in .bashrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `compass bash test ${randomUUID()}`);
  ...
  describe('removeBashrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '  . ~/.local/share/bash-completion/completions/compass',
        'fi',
        '# COMPASS:END',
        '',
        'alias ll="ls -la"',
      ...
      expect(newContent).not.toContain('Compass shell completions configuration');
