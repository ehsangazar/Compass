...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `compass-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'CompassCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'CompassCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'CompassCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/CompassCompletion.ps1';

    // Note: COMPASS_NO_AUTO_CONFIG check is now handled in the install() method,
    ...
    it.skipIf(process.platform === 'win32')('should skip configuration when script line already exists', async () => {
      ...
      const initialContent = [
        '# COMPASS:START - Compass completion (managed block, do not edit manually)',
        `. "${mockScriptPath}"`,
        '# COMPASS:END',
        '',
        '# My custom config',
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# COMPASS:END');
      expect(content).not.toContain('# Compass completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for Compass
$compassCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName compass -ScriptBlock $compassCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('CompassCompletion.ps1');
    ...
    // Note: COMPASS_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `compass powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('compass powershell test');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      // Verify content: original line kept, Compass block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install → uninstall without corruption', async () => {
      ...
      // Install adds the Compass block
      ...
      // Uninstall removes the Compass block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# COMPASS:START');
      ...
      expect(profileContentAfter).not.toContain('# COMPASS:START');
    });
