...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Compass CLI');
			expect(script).toContain('$compassCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for compass command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Compass',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName compass');
			expect(script).toContain('-ScriptBlock $compassCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-CompassChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-CompassSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-CompassSchemas');
			expect(script).toContain('compass __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-CompassChanges');
			expect(script).toContain('compass __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-CompassSpecs');
			expect(script).toContain('compass __complete specs 2>$null');
		});
