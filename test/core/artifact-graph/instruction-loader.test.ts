...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'compass-test-'));
    });
    ...
    it('should detect completed artifacts', () => {
      ...
      const changeDir = path.join(tempDir, 'compass', 'changes', 'my-change');
    ...
    it('should auto-detect schema from .compass.yaml metadata', () => {
      // Create change directory with metadata file
      const changeDir = path.join(tempDir, 'compass', 'changes', 'my-change');
      fs.mkdirSync(changeDir, { recursive: true });
      fs.writeFileSync(path.join(changeDir, '.compass.yaml'), 'schema: spec-driven\ncreated: "2025-01-05"\n');
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        const freshTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'compass-test-'));

        try {
          // Create project config with a uniquely named invalid artifact ID
          const configDir = path.join(freshTempDir, 'compass');
          fs.mkdirSync(configDir, { recursive: true });
          fs.writeFileSync(
            path.join(configDir, 'config.yaml'),
            `schema: spec-driven
