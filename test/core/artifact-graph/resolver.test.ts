...
describe('artifact-graph/resolver', () => {
  ...
  beforeEach(() => {
    tempDir = path.join(os.tmpdir(), `compass-resolver-test-${Date.now()}`);
  ...
  describe('getUserSchemasDir', () => {
    it('should use XDG_DATA_HOME when set', () => {
      ...
      expect(userDir).toBe(path.join(tempDir, 'compass', 'schemas'));
    });
  });
  ...
  describe('getSchemaDir', () => {
    ...
    it('should prefer user override directory', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'spec-driven');
...
`;
  ...
  describe('listSchemas', () => {
    ...
    it('should include user override schemas', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'custom-workflow');
      fs.mkdirSync(userSchemaDir, { recursive: true });
    ...
    it('should only include directories with schema.yaml', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemasBase = path.join(tempDir, 'compass', 'schemas');
  ...
  describe('getProjectSchemasDir', () => {
    it('should return correct path', () => {
      ...
      expect(schemasDir).toBe(path.join('/path/to/project', 'compass', 'schemas'));
    });

    it('should work with relative-looking paths', () => {
      const schemasDir = getProjectSchemasDir('./my-project');
      expect(schemasDir).toBe(path.join('my-project', 'compass', 'schemas'));
    });
  });
  ...
  describe('getSchemaDir with projectRoot', () => {
    ...
    it('should prefer project-local schema over user override', () => {
      ...
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'my-schema');
      fs.mkdirSync(userSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(userSchemaDir, 'schema.yaml'),
        'name: user-version\nversion: 1\nartifacts: []'
      ...
      const projectSchemaDir = path.join(projectRoot, 'compass', 'schemas', 'my-schema');
    ...
    it('should prefer project-local schema over package built-in', () => {
      ...
      const projectRoot = path.join(tempDir, 'project');
      const projectSchemaDir = path.join(projectRoot, 'compass', 'schemas', 'spec-driven');
      fs.mkdirSync(projectSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(projectSchemaDir, 'schema.yaml'),
    ...
    it('should fall back to user override when no project-local schema', () => {
      ...
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'user-only-schema');
      fs.mkdirSync(userSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(userSchemaDir, 'schema.yaml'),
        'name: user-only\nversion: 1\nartifacts: []'
  ...
  describe('resolveSchema with projectRoot', () => {
    it('should resolve project-local schema', () => {
      const projectRoot = path.join(tempDir, 'project');
      const projectSchemaDir = path.join(projectRoot, 'compass', 'schemas', 'team-workflow');
      fs.mkdirSync(projectSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(projectSchemaDir, 'schema.yaml'),
        `name: team-workflow
...
`
    ...
    it('should prefer project-local over user override when resolving', () => {
      // Set up user override
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'shared-schema');
      fs.mkdirSync(userSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(userSchemaDir, 'schema.yaml'),
        `name: user-version
...
`
      ...
      // Set up project-local schema
      const projectRoot = path.join(tempDir, 'project');
      const projectSchemaDir = path.join(projectRoot, 'compass', 'schemas', 'shared-schema');
      fs.mkdirSync(projectSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(projectSchemaDir, 'schema.yaml'),
        `name: project-version
...
`
  ...
  describe('listSchemas with projectRoot', () => {
    ...
    it('should maintain backward compatibility when projectRoot not provided', () => {
      // Set up project-local schema
      const projectRoot = path.join(tempDir, 'project');
      const projectSchemaDir = path.join(projectRoot, 'compass', 'schemas', 'project-only');
      fs.mkdirSync(projectSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(projectSchemaDir, 'schema.yaml'),
        'name: project-only\nversion: 1\nartifacts: []'
...
`
    ...
    it('should return source: user for user override schemas', () => {
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'user-custom');
      fs.mkdirSync(userSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(userSchemaDir, 'schema.yaml'),
        `name: user-custom
...
`
    ...
    it('should show project source when project-local shadows user override', () => {
      // Set up user override
      process.env.XDG_DATA_HOME = tempDir;
      const userSchemaDir = path.join(tempDir, 'compass', 'schemas', 'shared');
      fs.mkdirSync(userSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(userSchemaDir, 'schema.yaml'),
        `name: user-shared
...
`
      ...
      const projectRoot = path.join(tempDir, 'project');
      const projectSchemaDir = path.join(projectRoot, 'compass', 'schemas', 'shared');
      fs.mkdirSync(projectSchemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(projectSchemaDir, 'schema.yaml'),
        `name: project-shared
