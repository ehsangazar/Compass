...
describe('project-config', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'compass-test-config-'));
...
`
      ...
      it('should handle completely invalid YAML gracefully', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining('Failed to parse compass/config.yaml'),
          expect.anything()
        );
      });
...
`
    ...
    describe('.yml/.yaml precedence', () => {
      ...
      it('should return null when compass directory does not exist', () => {
        const config = readProjectConfig(tempDir);
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit compass/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });
