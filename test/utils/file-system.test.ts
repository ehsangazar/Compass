  ...
  describe('joinPath', () => {
    it('should join POSIX-style paths', () => {
      const result = FileSystemUtils.joinPath(
        '/tmp/project',
        '.claude/commands/compass/proposal.md'
      );
      expect(result).toBe('/tmp/project/.claude/commands/compass/proposal.md');
    });

    it('should join Linux home directory paths', () => {
      const result = FileSystemUtils.joinPath(
        '/home/dev/workspace/compass',
        '.cursor/commands/install.md'
      );
      expect(result).toBe('/home/dev/workspace/compass/.cursor/commands/install.md');
    });
    ...
    it('should join Windows drive-letter paths with backslashes', () => {
      ...
      expect(result).toBe(
        'C:\\Users\\dev\\project\\.claude\\commands\\compass\\proposal.md'
      );
    });
    ...
    it('should join Windows paths that use forward slashes', () => {
      ...
      expect(result).toBe(
        'D:\\workspace\\app\\.cursor\\commands\\compass-apply.md'
      );
    });
    ...
    it('should join UNC-style Windows paths', () => {
      const result = FileSystemUtils.joinPath(
        '\\server\\share\\repo',
        '.windsurf/workflows/compass-archive.md'
      );
      expect(result).toBe(
        '\\server\\share\\repo\\.windsurf\\workflows\\compass-archive.md'
      );
    });
  });
});
