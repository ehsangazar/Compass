...
describe('config command integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `compass-config-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('should use XDG_CONFIG_HOME for config path', async () => {
    ...
    expect(configPath).toBe(path.join(tempDir, 'compass', 'config.json'));
  });
...
describe('config command shell completion registry', () => {
  it('should have config command in registry', async () => {
    ...
    expect(configCmd?.description).toBe('View and modify global Compass configuration');
  });
...
describe('config profile command', () => {
  ...
  beforeEach(() => {
    tempDir = path.join(os.tmpdir(), `compass-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    fs.mkdirSync(tempDir, { recursive: true });
    originalEnv = { ...process.env };
    process.env.XDG_CONFIG_HOME = tempDir;
  });
