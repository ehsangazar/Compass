...
describe('top-level show command', () => {
  ...
  const changesDir = path.join(testDir, 'compass', 'changes');
  ...
  it('prints hint and non-zero exit when no args and non-interactive', () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${compassBin} show`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
      expect(stderr).toContain('Nothing to show.');
      expect(stderr).toContain('compass show <item>');
      expect(stderr).toContain('compass change show');
      expect(stderr).toContain('compass spec show');
    } finally {
  ...
  it('auto-detects change id and supports --json', () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${compassBin} show demo --json`, { encoding: 'utf-8' });
  ...
  it('auto-detects spec id and supports spec-only flags', () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${compassBin} show auth --json --requirements`, { encoding: 'utf-8' });
      const json = JSON.parse(output);
  ...
  it('handles ambiguity and suggests --type', async () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${compassBin} show foo`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
  ...
  it('prints nearest matches when not found', () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${compassBin} show unknown-item`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
