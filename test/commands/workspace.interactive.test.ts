...
async function runWorkspaceCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'compass', 'workspace', ...args]);
}
...
describe('workspace command interactive flows', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'compass-workspace-interactive-'));
  ...
  it('asks which agents get Compass skills and preselects the preferred opener', async () => {
    const api = mkdir('repos/api');
    const binDir = mkdir('bin');
    ...
    searchableMultiSelectMock.mockImplementationOnce(async (options: {
      ...
      expect(options.message).toBe('Which agents should get Compass skills in this workspace?');
