...
describe('schema command', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(
      os.tmpdir(),
      `compass-schema-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );
    fs.mkdirSync(tempDir, { recursive: true });

    // Create compass directory structure
    fs.mkdirSync(path.join(tempDir, 'compass', 'schemas'), { recursive: true });
  ...
  describe('schema which', () => {
    ...
    it('should detect project schema shadowing package', async () => {
      ...
      const projectSchemaDir = path.join(tempDir, 'compass', 'schemas', 'spec-driven');
...
`
  ...
  describe('schema validate', () => {
    it('should validate a valid schema', async () => {
      ...
      const schemaDir = path.join(tempDir, 'compass', 'schemas', 'test-schema');
...
`
    ...
    it('should detect missing template file', async () => {
      const schemaDir = path.join(tempDir, 'compass', 'schemas', 'bad-schema');
      fs.mkdirSync(schemaDir, { recursive: true });
      fs.writeFileSync(
        path.join(schemaDir, 'schema.yaml'),
        `name: bad-schema
...
`;
  ...
  describe('schema fork', () => {
    it('should copy schema to project directory', async () => {
      ...
      const destDir = path.join(tempDir, 'compass', 'schemas', 'my-custom');
  ...
  describe('schema init', () => {
    it('should create schema directory with schema.yaml', async () => {
      const schemaDir = path.join(tempDir, 'compass', 'schemas', 'new-schema');
      fs.mkdirSync(schemaDir, { recursive: true });
