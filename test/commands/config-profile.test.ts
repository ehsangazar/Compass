...
async function runConfigCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'compass', 'config', ...args]);
}
...
describe('config profile interactive flow', () => {
  ...
  function setupDriftedProjectArtifacts(projectDir: string): void {
    fs.mkdirSync(path.join(projectDir, 'compass'), { recursive: true });
    const exploreSkillPath = path.join(projectDir, '.claude', 'skills', 'compass-explore', 'SKILL.md');
    fs.mkdirSync(path.dirname(exploreSkillPath), { recursive: true });
    fs.writeFileSync(exploreSkillPath, 'name: compass-explore\n', 'utf-8');
  }
  ...
  function setupSyncedCoreBothArtifacts(projectDir: string): void {
    ...
    for (const commandId of coreCommands) {
      const commandPath = path.join(projectDir, '.claude', 'commands', 'compass', `${commandId}.md`);
  ...
  function addExtraVerifyWorkflowArtifacts(projectDir: string): void {
    const verifySkillPath = path.join(projectDir, '.claude', 'skills', 'compass-verify-change', 'SKILL.md');
    fs.mkdirSync(path.dirname(verifySkillPath), { recursive: true });
    fs.writeFileSync(verifySkillPath, 'name: compass-verify-change\n', 'utf-8');

    const verifyCommandPath = path.join(projectDir, '.claude', 'commands', 'compass', 'verify.md');
  ...
  function setupWorkspaceState(
    ...
    const metadataDir = path.join(workspaceRoot, '.compass-workspace');
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `compass-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('selecting current values only should be a no-op and should not ask apply', async () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'compass'), { recursive: true });
    select.mockResolvedValueOnce('delivery');
  ...
  it('changed config should ask to apply to the current workspace and print workspace guidance when declined', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith('Config updated. Run `compass workspace update` to apply it to workspace-local skills.');
  });
  ...
  it('confirmed workspace apply should run workspace update instead of repo-local update', async () => {
    ...
    expect(execSync).toHaveBeenCalledWith('npx compass workspace update', {
      stdio: 'inherit',
      cwd: process.cwd(),
    });
    expect(execSync).not.toHaveBeenCalledWith('npx compass update', expect.anything());
  });
  ...
  it('no-op inside a workspace should warn when workspace skills drift', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('compass workspace update'));
  });
