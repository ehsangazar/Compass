...
describe('workspace command', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'compass-workspace-command-'));
  ...
  function createFakeExecutable(name: string): { binDir: string; logPath: string } {
    ...
    fs.writeFileSync(
      recorderPath,
      "const fs = require('node:fs');\nfs.writeFileSync(process.env.COMPASS_FAKE_OPEN_LOG, JSON.stringify({ cwd: process.cwd(), args: process.argv.slice(2) }));\n"
    );

    const posixExecutable = path.join(binDir, name);
    fs.writeFileSync(posixExecutable, '#!/bin/sh\nnode "$COMPASS_FAKE_OPEN_RECORDER" "$@"\n');
  ...
  function envWithFakeExecutable(fake: { binDir: string; logPath: string }): NodeJS.ProcessEnv {
    return {
      ...
      COMPASS_FAKE_OPEN_RECORDER: path.join(fake.binDir, 'record-launch.cjs'),
      COMPASS_FAKE_OPEN_LOG: fake.logPath,
    };
  }
  ...
  function writeGlobalConfig(config: Record<string, unknown>): void {
    const configDir = path.join(configHome, 'compass');
  ...
  it('sets up a workspace with required links, records local state, and lists it through ls', async () => {
    ...
    expect(setup.workspace.links).toEqual([
      expect.objectContaining({
        name: 'api',
        path: expectedApi,
        repo_specs_path: path.join(expectedApi, 'compass', 'specs'),
        status: [],
      }),
      expect.objectContaining({
        name: 'checkout',
    ...
    const registry = parseWorkspaceRegistryState(
      fs.readFileSync(
        getWorkspaceRegistryPath({ globalDataDir: path.join(dataHome, 'compass') }),
        'utf-8'
      )
    );
      ...
      expect.objectContaining({
        ...
        skipped: [
          expect.objectContaining({
            ...
            message: expect.stringContaining('compass workspace update --tools <ids>'),
          }),
        ],
      })
    );
  ...
  it('installs profile-selected workspace skills in the workspace root only', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-apply-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-archive-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-propose', 'SKILL.md'))).toBe(false);
  ...
  it('updates stored workspace skills from the current workspace and clears profile drift', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-apply-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-verify-change', 'SKILL.md'))).toBe(true);
    ...
    expect(parseJson(drift).workspace.status).toContainEqual(
      expect.objectContaining({
        ...
        fix: 'compass workspace update --workspace profile-sync',
      })
    );
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-propose', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-explore', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-sync-specs', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-archive-change', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-verify-change'))).toBe(false);
  ...
  it('redirects compass update from a workspace planning home to workspace update', async () => {
    const api = mkdir('repos/api');
    const linkedEntriesBefore = fs.readdirSync(api).sort();
    writeGlobalConfig({
      profile: 'custom',
  ...
  it('updates the workspace passed to compass update even when another workspace is known', async () => {
    ...
    expect(update.stdout).not.toContain('Multiple Compass workspaces are known');
    expect(fs.existsSync(path.join(first.workspace.root, '.codex', 'skills', 'compass-propose', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(second.workspace.root, '.codex', 'skills', 'compass-propose', 'SKILL.md'))).toBe(false);
  });
  ...
  it('supports named and flag-selected workspace updates with explicit agent changes', async () => {
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.claude', 'skills', 'compass-apply-change', 'SKILL.md'))).toBe(true);
    ...
    expect(fs.existsSync(path.join(workspaceRoot, '.codex', 'skills', 'compass-apply-change'))).toBe(false);
  ...
  it('does not remove unmanaged skill directories that collide with Compass workflow names', async () => {
    const api = mkdir('repos/api');
    writeGlobalConfig({
      profile: 'custom',
      delivery: 'skills',
    ...
    const workspaceRoot = setup.workspace.root;
    const collidingSkillDir = path.join(workspaceRoot, '.codex', 'skills', 'compass-verify-change');
  ...
  it('does not record workspace skills as applied when an update fails', async () => {
    ...
    const workspaceRoot = setup.workspace.root;
    const blockingSkillPath = path.join(workspaceRoot, '.codex', 'skills', 'compass-propose');
  ...
  it('rejects duplicate setup link names without creating or rewriting a workspace', async () => {
    ...
    expect(fs.existsSync(getWorkspaceRegistryPath({ globalDataDir: path.join(dataHome, 'compass') }))).toBe(false);
  });
  ...
  it('removes a partially created workspace when setup fails after creating the root', async () => {
    ...
    const globalDataDir = path.join(dataHome, 'compass');
  ...
  it('fails setup cleanly for missing automation inputs and JSON without no-interactive', async () => {
    ...
    expect(noWorkspaces.stdout).toContain("No Compass workspaces found. Run 'compass workspace setup' first.");
  ...
  it('links monorepo folders without editing the linked folder', async () => {
    ...
    expect(fs.existsSync(path.join(packageDir, 'compass'))).toBe(false);
  ...
  it('reports stale registry entries without rewriting the registry', async () => {
    const api = mkdir('repos/api');
    const setup = await setupWorkspace('platform', [`api=${api}`]);
    const registryPath = getWorkspaceRegistryPath({ globalDataDir: path.join(dataHome, 'compass') });
...
`;
  ...
  it('fails non-interactive ambiguous workspace selection in human output mode', async () => {
    ...
    expect(doctor.exitCode).toBe(1);
    expect(doctor.stderr).toContain('Multiple Compass workspaces are known.');
    expect(doctor.stderr).toContain('Pass --workspace <name>.');
    expect(doctor.stderr).toContain('compass workspace doctor --workspace <name>');
  });
  ...
  it('reports workspace open selection, unsupported flag, unset opener, and unavailable opener errors', async () => {
    ...
    expect(noKnown.exitCode).toBe(1);
    expect(noKnown.stderr).toContain("No known Compass workspaces. Run 'compass workspace setup' first.");
...
`
  ...
  it('prints readable human output for setup, list, and doctor', async () => {
    ...
    expect(setup.stdout).toContain('Compass workspaces (1)');
    ...
    expect(list.exitCode).toBe(0);
    expect(list.stdout).toContain('Compass workspaces (1)');
