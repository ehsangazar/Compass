...
async function prepareFixture(fixtureName: string): Promise<string> {
  const base = await fs.mkdtemp(path.join(tmpdir(), 'compass-cli-e2e-'));
  tempRoots.push(base);
...
describe('compass CLI e2e basics', () => {
  it('shows help output', async () => {
    const result = await runCLI(['--help']);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Usage: compass');
    expect(result.stderr).toBe('');
  ...
  describe('init command non-interactive options', () => {
    it('initializes with --tools all option', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('Compass Setup Complete');

      // Check that skills were created for multiple tools
      const claudeSkillPath = path.join(emptyProjectDir, '.claude/skills/compass-explore/SKILL.md');
      const cursorSkillPath = path.join(emptyProjectDir, '.cursor/skills/compass-explore/SKILL.md');
