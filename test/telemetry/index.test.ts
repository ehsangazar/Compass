...
describe('telemetry/index', () => {
  ...
  describe('isTelemetryEnabled', () => {
    it('should return false when COMPASS_TELEMETRY=0', () => {
      process.env.COMPASS_TELEMETRY = '0';
    ...
    it('should return true when no opt-out is set', () => {
      delete process.env.COMPASS_TELEMETRY;
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(true);
    });

    it('should prioritize COMPASS_TELEMETRY=0 over other settings', () => {
      process.env.COMPASS_TELEMETRY = '0';
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(false);
  ...
  describe('trackCommand', () => {
    ...
    it('should return a synthetic success response when fetch throws a network error', async () => {
      ...
      const response = await fetchFn('https://edge.compass.dev/batch/', { method: 'POST' });
