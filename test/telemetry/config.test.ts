...
describe('telemetry/config', () => {
  ...
  function defaultConfigDir(): string {
    return os.platform() === 'win32'
      ? path.join(tempDir, 'appdata', 'compass')
      : path.join(tempDir, '.config', 'compass');
  }
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `compass-telemetry-test-${randomUUID()}`);
  ...
  describe('getConfigPath', () => {
    ...
    it('should use XDG_CONFIG_HOME when set', () => {
      ...
      expect(result).toBe(path.join(xdgConfigHome, 'compass', 'config.json'));
    });
  });
  ...
  describe('readConfig', () => {
    ...
    it('should migrate telemetry from legacy path when XDG_CONFIG_HOME is set', async () => {
      const xdgConfigHome = path.join(tempDir, 'xdg-config');
      const legacyConfigDir = path.join(tempDir, '.config', 'compass');
      const legacyConfigPath = path.join(legacyConfigDir, 'config.json');
      const newConfigPath = path.join(xdgConfigHome, 'compass', 'config.json');
      process.env.XDG_CONFIG_HOME = xdgConfigHome;
    ...
    it('should not overwrite invalid new config during legacy migration', async () => {
      ...
      const legacyConfigDir = path.join(tempDir, '.config', 'compass');
      const legacyConfigPath = path.join(legacyConfigDir, 'config.json');
      const newConfigDir = path.join(xdgConfigHome, 'compass');
  ...
  describe('writeConfig', () => {
    ...
    it('should write config to XDG_CONFIG_HOME when set', async () => {
      const xdgConfigHome = path.join(tempDir, 'xdg-config');
      const configPath = path.join(xdgConfigHome, 'compass', 'config.json');
      process.env.XDG_CONFIG_HOME = xdgConfigHome;
